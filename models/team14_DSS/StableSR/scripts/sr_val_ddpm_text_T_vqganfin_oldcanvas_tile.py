...
def main():
	...
	parser.add_argument(
		...
		default="../DiffBIR/result_diffbir"
	)
	parser.add_argument(
        "--model_dir",
        type=str,
	...
	parser.add_argument(
		"--ckpt",
		type=str,
		default="checkpoint_stablesr/stablesr_turbo.ckpt",
		help="path to checkpoint of model",
	)
	parser.add_argument(
		"--vqgan_ckpt",
		type=str,
		default="checkpoint_stablesr/vqgan_cfw_00011.ckpt",
	...
	opt.ckpt = os.path.join(opt.model_dir, 'stablesr', 'stablesr_turbo.ckpt')					####################################
	opt.vqgan_ckpt = os.path.join(opt.model_dir, 'stablesr', 'vqgan_cfw_00011.ckpt')

	config = OmegaConf.load(f"{opt.config}")
	config.model.params.cond_stage_config.params.pretrained_path = os.path.join(opt.model_dir, 'stablesr', 'CLIP-ViT-H-14-laion2B-s32B-b79K', 'open_clip_pytorch_model.bin')	####################################
